...
function interpolateQueryWithValues(query, values) {
  try{
  ...
  function wrapQuery (orig, name) {
    return function wrappedFunction (sql) {
      ...
      if (typeof sqlText === 'string') {
        ...
        const parsedQuery = interpolateQueryWithValues(sqlText, arguments[1]);

        span.name = sqlSummary(sqlText)
        dbContext.statement = parsedQuery === sqlText ? sqlText : ("original: " + sqlText + (parsedQuery ? ("\n" + "parsed: " + parsedQuery) : ''));
      } else {
